...
**Kết luận trung thực**: cải thiện của blend lần này thực chất chỉ là "gần như triển khai CatBoost" khoác áo blend, chứ không phải trường hợp kết hợp hai cách xếp hạng khác nhau tạo ra giá trị vượt trội so với CatBoost một mình. Kết hợp với phát hiện ở 5.4 (CatBoost AUC-tuned đang là model tốt nhất ở 58.0%), bức tranh chung qua cả 3 phiên bản feature set của notebook này là: model cây thỉnh thoảng nhỉnh hơn Logistic Regression ở Recall@Top-15%, mức độ dao động theo từng lần thêm feature, còn Logistic Regression vẫn được giữ lại để triển khai vì lý do interpretability (Section 6.4).
...
- Đã rà soát toàn bộ cột dữ liệu sẵn có: thêm `dominant_weather_event_type`, 3 cột carrier, và thử thêm `n_shipments_same_origin_day` (proxy tắc nghẽn depot) — tất cả vượt qua kiểm tra redundancy, nhưng chỉ `dominant_weather_event_type` mang tín hiệu biên thật; 3 cột carrier và feature tắc nghẽn đều không cải thiện gì đáng kể. Muốn tăng recall thêm cần **nguồn dữ liệu mới** thực sự khác biệt (ví dụ traffic thời gian thực), không phải thêm cột phái sinh từ dữ liệu đã có hay tune/kết hợp model nhiều hơn (đã thử ở `experiments.ipynb`, kết quả không đồng nhất/không đáng tin cậy).
